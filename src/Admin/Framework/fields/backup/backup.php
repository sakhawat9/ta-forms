 ...
 *
 */
if ( ! class_exists( 'Ta_Forms_Field_backup' ) ) {
  class Ta_Forms_Field_backup extends Ta_Forms_Fields {
    ...
    public function render() {
      ...
      $unique = $this->unique;
      $nonce  = wp_create_nonce( 'Ta_Forms_backup_nonce' );
      $export = add_query_arg( array( 'action' => 'ta-forms-export', 'unique' => $unique, 'nonce' => $nonce ), admin_url( 'admin-ajax.php' ) );

      echo wp_kses_post($this->field_before());

      echo '<textarea name="Ta_Forms_import_data" class="ta-forms-import-data"></textarea>';
      ...
      echo '<hr />';
      echo '<button type="submit" name="Ta_Forms_transient[reset]" value="reset" class="button ta-forms-warning-primary ta-forms-confirm ta-forms-reset" data-unique="'. esc_attr( $unique ) .'" data-nonce="'. esc_attr( $nonce ) .'">'. esc_html__( 'Reset', 'ta-forms' ) .'</button>';
