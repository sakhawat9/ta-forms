  ...
  class Ta_Forms_Field_select extends Ta_Forms_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' ta-forms-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' ta-forms-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. esc_attr( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="ta-forms-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select style="min-width:300px" name="'. $field_name .'" class="ta-forms-hide-select hidden"'. $multiple_attr . $field_attr .'>';
          ...
          echo '<select style="min-width:300px" name="'. esc_attr( $field_name ) .'"'. $field_class . $multiple_attr . $placeholder_attr . $field_attr . $chosen_data_attr .'>';
