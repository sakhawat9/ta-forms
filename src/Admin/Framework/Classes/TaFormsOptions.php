  ...
  class TaFormsOptions extends Ta_Forms_Abstract
  {
    ...
    public function add_admin_bar_menu($wp_admin_bar)
    {
      ...
      if (! empty($this->args['show_bar_menu']) && empty($this->args['menu_hidden'])) {
        ...
        $menu_icon = (! empty($this->args['admin_bar_menu_icon'])) ? '<span class="ta-forms-ab-icon ab-icon ' . esc_attr($this->args['admin_bar_menu_icon']) . '"></span>' : '';
    ...
    public function ajax_save()
    {
      ...
      if (! $result) {
        wp_send_json_error(array('error' => esc_html__('Error while saving the changes.', 'ta-forms')));
      } else {
    ...
    public function set_options($ajax = false)
    {
      ...
      if (wp_verify_nonce($nonce, 'Ta_Forms_options_nonce')) {
        ...
        if (! empty($transient['reset'])) {
          ...
          $this->notice = esc_html__('Default settings restored.', 'ta-forms');
        } else if (! empty($transient['reset_section']) && ! empty($section_id)) {

          if (!empty($this->pre_sections)) {
            foreach ($this->pre_sections as $key => $fields) {
              $fields_id = strval($key + 1);
              if ($fields_id === $section_id) {
          ...
          foreach ($sections['fields'] as $field) {
            if (! empty($field['id'])) {
              if (array_key_exists($field['id'], $this->errors)) {
                $err = '<span class="ta-forms-label-error">!</span>';
              }
            }
          }
        }
    ...
    public function add_options_html()
    {
      ...
      $show_all      = (! $has_nav) ? ' ta-forms-show-all' : '';
      $ajax_class    = ($this->args['ajax_save']) ? ' ta-forms-save-ajax' : '';
      $sticky_class  = ($this->args['sticky_header']) ? ' ta-forms-sticky-header' : '';
      $wrapper_class = ($this->args['framework_class']) ? ' ' . $this->args['framework_class'] : '';
      $theme         = ($this->args['theme']) ? ' ta-forms-theme-' . $this->args['theme'] : '';
      ...
      echo '<div class="ta-forms-admin-header">';

      echo '<div class="ta-forms-header-left">';
      echo '<img src="'.esc_url(TAFORMS_DIR_URL .'src/Admin/Framework/assets/images/logo.png').'"/>';
      echo '<h1>' . esc_html($this->args['framework_title']) . '</h1>';
      echo '<div class="ta-forms-version">v'.TAFORMS_VERSION.'</div>';
      echo '</div>';

      echo '<div class="ta-forms-header-right">';

      $notice_class = (! empty($this->notice)) ? 'ta-forms-form-show' : '';
      $notice_text  = (! empty($this->notice)) ? $this->notice : '';

      echo '<div class="ta-forms-form-result ta-forms-form-success ' . esc_attr($notice_class) . '">' . esc_html($notice_text) . '</div>';

      echo ($has_nav && $this->args['show_all_options']) ? '<div class="ta-forms-expand-all" title="' . esc_html__('show all settings', 'ta-forms') . '"><i class="icofont-outdent"></i></div>' : '';

      echo ($this->args['show_search']) ? '<div class="ta-forms-search"><input type="text" name="ta-forms-search" placeholder="' . esc_html__('Search...', 'ta-forms') . '" autocomplete="off" /></div>' : '';
      ...
      echo '<div class="ta-forms-wrapper' . esc_attr($show_all) . '">';

      if ($has_nav) {

        echo '<div class="ta-forms-nav ta-forms-nav-' . esc_attr($nav_type) . ' ta-forms-nav-options">';
        ...
        foreach ($this->pre_tabs as $tab) {
          ...
          $tab_icon  = (! empty($tab['icon'])) ? '<i class="ta-forms-tab-icon ' . esc_attr($tab['icon']) . '"></i>' : '';

          if (! empty($tab['subs'])) {

            echo '<li class="ta-forms-tab-item">';

            echo '<a href="#tab=' . esc_attr($tab_id) . '" data-tab-id="' . esc_attr($tab_id) . '" class="ta-forms-arrow">' . $tab_icon . $tab['title'] . $tab_error . '</a>';
            ...
            foreach ($tab['subs'] as $sub) {
              ...
              $sub_icon  = (! empty($sub['icon'])) ? '<i class="ta-forms-tab-icon ' . esc_attr($sub['icon']) . '"></i>' : '';
            ...
            echo '<li class="ta-forms-tab-item"><a href="#tab=' . esc_attr($tab_id) . '" data-tab-id="' . esc_attr($tab_id) . '">' . $tab_icon . $tab['title'] . $tab_error . '</a></li>';
          }
        }
      ...
      echo '<div class="ta-forms-content">';

      echo '<div class="ta-forms-sections">';

      foreach ($this->pre_sections as $section) {

        $section_onload = (! $has_nav) ? ' ta-forms-onload' : '';
        $section_class  = (! empty($section['class'])) ? ' ' . $section['class'] : '';
        $section_icon   = (! empty($section['icon'])) ? '<i class="ta-forms-section-icon ' . esc_attr($section['icon']) . '"></i>' : '';
        ...
        echo '<div class="ta-forms-section hidden' . esc_attr($section_onload . $section_class) . '" data-section-id="' . esc_attr($section_parent . $section_slug) . '">';
        echo ($has_nav) ? '<div class="ta-forms-section-title"><h3>' . $section_icon . $section_title . '</h3></div>' : '';
        echo (! empty($section['description'])) ? '<div class="ta-forms-field ta-forms-section-description">' . $section['description'] . '</div>' : '';
        ...
        if (! empty($section['fields'])) {
          ...
          foreach ($section['fields'] as $field) {
            ...
            TaForms::field($field, $value, $this->unique, 'options');
          }
        } else {

          echo '<div class="ta-forms-no-option">' . esc_html__('No data available.', 'ta-forms') . '</div>';
        }
      ...
      echo ($has_nav && $nav_type === 'normal') ? '<div class="ta-forms-nav-background"></div>' : '';
