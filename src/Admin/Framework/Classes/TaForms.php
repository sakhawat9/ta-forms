...
namespace ThemeAtelier\TaForms\Admin\Framework\Classes;

if (! class_exists('TaForms')) {
  class TaForms
  {

    // Default constants
    public static $premium  = true;
    public static $version  = TAFORMS_VERSION;
    public static $dir      = '';
    public static $url      = '';
    public static $css      = '';
    public static $file     = '';
    ...
    public function __construct()
    {
      ...
      add_action('switch_theme', array('ThemeAtelier\TaForms\Admin\Framework\Classes\TaForms', 'setup'));
      add_action('admin_enqueue_scripts', array('ThemeAtelier\TaForms\Admin\Framework\Classes\TaForms', 'add_admin_enqueue_scripts'));
      add_action('wp_enqueue_scripts', array('ThemeAtelier\TaForms\Admin\Framework\Classes\TaForms', 'add_typography_enqueue_styles'), 80);
      add_action('wp_head', array('ThemeAtelier\TaForms\Admin\Framework\Classes\TaForms', 'add_custom_css'), 80);
      add_filter('admin_body_class', array('ThemeAtelier\TaForms\Admin\Framework\Classes\TaForms', 'add_admin_body_class'));
    }
    ...
    public static function setup()
    {
      ...
      $params = array();
      if (class_exists('TaFormsOptions') && ! empty(self::$args['admin_options'])) {
        foreach (self::$args['admin_options'] as $key => $value) {
          if (! empty(self::$args['sections'][$key]) && ! isset(self::$inited[$key])) {
            ...
            \TaFormsOptions::instance($key, $params);
      ...
      $params = array();
      if (class_exists('TaFormsMetabox') && ! empty(self::$args['metabox_options'])) {
        foreach (self::$args['metabox_options'] as $key => $value) {
          if (! empty(self::$args['sections'][$key]) && ! isset(self::$inited[$key])) {
            ...
            \TaFormsMetabox::instance($key, $params);
          }
        }
      }
    ...
    public static function includes()
    {
      ...
      self::include_plugin_file('Classes/TaFormsOptions.php');
    ...
    public static function add_admin_enqueue_scripts()
    {
      ...
      if (! self::$enqueue) {
        ...
        if ($wpscreen->id === 'tools_page_ta-forms-welcome') {
          self::$enqueue = true;
        }
      }
      ...
      // Main style
      wp_enqueue_style('ta-forms', self::include_plugin_url('assets/css/style' . $min . '.css'), array(), self::$version, 'all');

      // Main RTL styles
      if (is_rtl()) {
        wp_enqueue_style('ta-forms-rtl', self::include_plugin_url('assets/css/style-rtl' . $min . '.css'), array(), self::$version, 'all');
      }

      // Main scripts
      wp_enqueue_script('ta-forms-plugins', self::include_plugin_url('assets/js/plugins' . $min . '.js'), array(), self::$version, true);
      ...
      wp_localize_script('ta-forms', 'Chat_Help_Pro_vars', array(
        ...
        'i18n'              => array(
          ...
          'searching_text'  => esc_html__('Searching...', 'ta-forms'),
          'no_results_text' => esc_html__('No results found.', 'ta-forms'),
        ),
      ));
    ...
    public static function add_typography_enqueue_styles()
    {
      ...
      if (! empty(self::$webfonts)) {
        ...
        if (! empty(self::$webfonts['enqueue'])) {
          ...
          wp_enqueue_style('ta-forms-google-web-fonts', esc_url(add_query_arg($query, '//fonts.googleapis.com/css')), array(), null);
        }
        ...
        if (! empty(self::$webfonts['async'])) {
          ...
          wp_enqueue_script('ta-forms-google-web-fonts', esc_url('//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js'), array(), null);

          wp_localize_script('ta-forms-google-web-fonts', 'WebFontConfig', array('google' => array('families' => $fonts)));
        }
      }
    }
    ...
    public static function field($field = array(), $value = '', $unique = '', $where = '', $parent = '')
    {
      ...
      if (! empty($field['_notice'])) {
        ...
        $field            = array();
        $field['content'] = esc_html__('Oops! Not allowed.', 'ta-forms') . ' <strong>(' . esc_html($field_type) . ')</strong>';
        $field['type']    = 'notice';
        $field['style']   = 'danger';
      }
      ...
      $is_pseudo  = (! empty($field['pseudo'])) ? ' ta-forms-pseudo-field' : '';
      ...
      if (! empty($field['dependency'])) {
        ...
        $visible = (! empty($depend_visible)) ? ' ta-forms-depend-visible' : ' ta-forms-depend-hidden';
      }

      // These attributes has been sanitized above.
      echo '<div class="ta-forms-field ta-forms-field-' . esc_attr($field_type . $is_pseudo . $class . $visible) . '"' . wp_kses_post($depend) . '>';
      ...
      if (! empty($field_type)) {
        if (! empty($field['title'])) {
          ...
          if (! empty($field['title_help']) || ! empty($field['title_video'])) {
            ...
            $title_help      = sprintf('<span class="ta-forms-help ta-forms-title-help">
							<span class="ta-forms-help-text' . esc_attr($video_help_attr) . '">%s</span>
          ...
          echo '<div class="ta-forms-title">';
          echo '<h4>' . wp_kses_post($field['title']) . $title_help . '</h4>'; // phpcs:ignore -- Attributes are escaped above.
          echo (! empty($field['subtitle'])) ? '<div class="ta-forms-subtitle-text">' . wp_kses_post($field['subtitle']) . '</div>' : '';
          echo '</div>';
        }

        echo (! empty($field['title'])) ? '<div class="ta-forms-fieldset">' : '';
        ...
        if (class_exists($classname)) {
          ...
          echo '<p>' . esc_html__('Field not found!', 'ta-forms') . '</p>';
        }
      } else {
        echo '<p>' . esc_html__('Field not found!', 'ta-forms') . '</p>';
      }
