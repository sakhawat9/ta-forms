...
; (function ($, window, document, undefined) {
  ...
  TaForms.helper = {
    ...
    name_nested_replace: function ($selector, field_id) {
      ...
      var checks = [];
      var regex = new RegExp(TaForms.helper.preg_quote(field_id + '[\\d+]'), 'g');
  ...
  $.fn.Chat_Help_Pro_nav_options = function () {
    return this.each(function () {
      ...
      $window.on('hashchange ta-forms.hashchange', function () {
        ...
        if ($link.length) {
          ...
          $link.closest('.ta-forms-tab-item').addClass('ta-forms-tab-expanded').siblings().removeClass('ta-forms-tab-expanded');
  ...
  $.fn.Chat_Help_Pro_page_templates = function () {
    if (this.length) {
      ...
      $(document).on('change', '.editor-page-attributes__template select, #page_template, .edit-post-post-status + div select', function () {
        ...
        $('.ta-forms-page-templates').removeClass('ta-forms-metabox-show').addClass('ta-forms-metabox-hide');
        $('.ta-forms-page-' + maybe_value.toLowerCase().replace(/[^a-zA-Z0-9]+/g, '-')).removeClass('ta-forms-metabox-hide').addClass('ta-forms-metabox-show');
  ...
  $.fn.Chat_Help_Pro_post_formats = function () {
    if (this.length) {
      ...
      $(document).on('change', '.editor-post-format select, #formatdiv input[name="post_format"]', function () {
        ...
        $('.ta-forms-post-formats').removeClass('ta-forms-metabox-show').addClass('ta-forms-metabox-hide');
        $('.ta-forms-post-format-' + maybe_value).removeClass('ta-forms-metabox-hide').addClass('ta-forms-metabox-show');
      ...
      var $this = $(this),
        ...
        stickyUpdate = function () {
          ...
          if (stickyTop <= offset && winWidth > 782) {
            ...
            $this.css({ height: $this.outerHeight() }).addClass('ta-forms-sticky');
          } else {
    ...
    return this.each(function () {
      ...
      $reset.on('click', function (e) {
        ...
        if (TaForms.vars.is_confirm) {
          ...
          window.wp.ajax.post('ta-forms-reset', {
      ...
      $import.on('click', function (e) {
        ...
        if (TaForms.vars.is_confirm) {
          ...
          window.wp.ajax.post('ta-forms-import', {
    ...
    return this.each(function () {
      ...
      var interval = setInterval(function () {
        if ($this.is(':visible')) {
          ...
          if (data_editor.theme !== 'default' && TaForms.vars.code_themes.indexOf(data_editor.theme) === -1) {
  ...
  $.fn.Chat_Help_Pro_field_gallery = function () {
    return this.each(function () {
      ...
      $this.on('click', '.ta-forms-button, .ta-forms-edit-gallery', function (e) {
  ...
  $.fn.Chat_Help_Pro_field_group = function () {
    return this.each(function () {
      ...
      var update_title_numbers = function ($selector) {
        $selector.find('.ta-forms-cloneable-title-number').each(function (index) {
          $(this).html(($(this).closest('.ta-forms-cloneable-item').index() + 1) + '.');
        });
      };
        ...
        activate: function (event, ui) {
          ...
          if ($panel.length && !$panel.data('opened')) {
            ...
            $.each(inputs, function (key, $input) {
              ...
              $input.on('change keyup ta-forms.keyup', function () {
      ...
      $wrapper.children('.ta-forms-cloneable-item').children('.ta-forms-cloneable-helper').on('click', '.ta-forms-cloneable-clone', event_clone);
      $group.children('.ta-forms-cloneable-hidden').children('.ta-forms-cloneable-helper').on('click', '.ta-forms-cloneable-clone', event_clone);
      ...
      $wrapper.children('.ta-forms-cloneable-item').children('.ta-forms-cloneable-helper').on('click', '.ta-forms-cloneable-remove', event_remove);
      $group.children('.ta-forms-cloneable-hidden').children('.ta-forms-cloneable-helper').on('click', '.ta-forms-cloneable-remove', event_remove);
    ...
    return this.each(function () {
      ...
      $this.on('click', '.ta-forms-icon-add', function (e) {
        ...
        if (!TaForms.vars.icon_modal_loaded) {
          ...
          window.wp.ajax.post('ta-forms-get-icons', {
            ...
            var $load = $modal.find('.ta-forms-modal-load').html(response.content);
            ...
            $load.on('click', 'i', function (e) {
              ...
              TaForms.vars.$icon_target.find('.ta-forms-icon-preview i').removeAttr('class').addClass(icon);
              TaForms.vars.$icon_target.find('.ta-forms-icon-preview').removeClass('hidden');
              TaForms.vars.$icon_target.find('.ta-forms-icon-remove').removeClass('hidden');
              TaForms.vars.$icon_target.find('input').val(icon).trigger('change');
            ...
            $modal.on('click', '.ta-forms-modal-close, .ta-forms-modal-overlay', function () {
              $modal.addClass('hidden');
            });
  ...
  $.fn.Chat_Help_Pro_field_map = function () {
    return this.each(function () {
      ...
      if (!$search_input.length) {
        $search_input = $('[data-depend-id="' + $this.find('.ta-forms--address-field').data('address-field') + '"]');
      }
      ...
      $search_input.autocomplete({
        ...
        create: function (event, ui) {
          $(this).autocomplete('widget').addClass('ta-forms-map-ui-autocomplate');
        }
      });
  ...
  $.fn.Chat_Help_Pro_field_link = function () {
    return this.each(function () {
      ...
      $edit.on('click', function (e) {
        ...
        $('#wp-link-target').prop('checked', ($this.find('.ta-forms--target').val() === '_blank'));
  ...
  $.fn.Chat_Help_Pro_field_media = function () {
    return this.each(function () {
      ...
      var $this = $(this),
        ...
        $auto_attributes = ($this.hasClass('ta-forms-assign-field-background')) ? $this.closest('.ta-forms-field-background').find('.ta-forms--auto-attributes') : false,
        wp_media_frame;
  ...
  $.fn.Chat_Help_Pro_field_repeater = function () {
    return this.each(function () {
      ...
      $wrapper.children('.ta-forms-repeater-item').children('.ta-forms-repeater-helper').on('click', '.ta-forms-repeater-clone', event_clone);
      $repeater.children('.ta-forms-repeater-hidden').children('.ta-forms-repeater-helper').on('click', '.ta-forms-repeater-clone', event_clone);
      ...
      $wrapper.children('.ta-forms-repeater-item').children('.ta-forms-repeater-helper').on('click', '.ta-forms-repeater-remove', event_remove);
      $repeater.children('.ta-forms-repeater-hidden').children('.ta-forms-repeater-helper').on('click', '.ta-forms-repeater-remove', event_remove);
  ...
  $.fn.Chat_Help_Pro_field_spinner = function () {
    return this.each(function () {
      ...
      $input.spinner({
        ...
        create: function (event, ui) {
          if (data.unit) {
            $input.after('<span class="ui-button ta-forms--unit">' + data.unit + '</span>');
          }
        },
  ...
  $.fn.Chat_Help_Pro_field_typography = function () {
    return this.each(function () {
      ...
      base.append_select_options = function ($select, options, condition, type, is_multi) {
        ...
        $select.append(opts).trigger('ta-forms.change').trigger('chosen:updated');
      ...
      base.init = function () {
        ...
        if ($font_style_block.length) {
          ...
          var $extra_font_style_block = $this.find('.ta-forms--block-extra-styles');

          if ($extra_font_style_block.length) {
            var $extra_font_style_select = $this.find('.ta-forms--extra-styles');
        ...
        //
        // Backup font family
        var $backup_font_family_block = $this.find('.ta-forms--block-backup-font-family');

        //
        //
        // Font Family on Change Listener
        $font_family_select.on('change ta-forms.change', function (event) {
        ...
        if ($preview_block.length) {
          ...
          $this.on('change', TaForms.helper.debounce(function (event) {
            ...
            var font_family = $font_family_select.val(),
              ...
              text_decoration = $this.find('.ta-forms--text-decoration').val(),
    ...
    return this.each(function () {
      ...
      if (field_editor_settings.media_buttons && window.Chat_Help_Pro_media_buttons) {
        ...
        if ($editor_buttons.length) {
          ...
          $editor_buttons.find('.ta-forms-shortcode-button').data('editor-id', uid);
            ...
            window.wp.ajax.post('Chat_Help_Pro_' + $panel.data('unique') + '_ajax_save', {
              ...
              .done(function (response) {
                ...
                if (Object.keys(response.errors).length) {
                  ...
                  var error_icon = '<i class="ta-forms-label-error ta-forms-error">!</i>';

                  $.each(response.errors, function (key, error_message) {

                    var $field = $('[data-depend-id="' + key + '"]'),
                      $link = $('a[href="#tab=' + $field.closest('.ta-forms-section').data('section-id') + '"]'),
                      $tab = $link.closest('.ta-forms-tab-item');

                    $field.closest('.ta-forms-fieldset').append('<p class="ta-forms-error ta-forms-error-text">' + error_message + '</p>');
  ...
  $.fn.Chat_Help_Pro_shortcode = function () {
    ...
    return this.each(function () {
      ...
      $(document).on('click', '.ta-forms-shortcode-button[data-modal-id="' + modal_id + '"]', function (e) {
        ...
        if ($modal.hasClass('ta-forms-shortcode-single') && sc_name === undefined) {
      ...
      $select.on('change', function () {
        ...
        if (sc_key) {
          ...
          window.wp.ajax.post('ta-forms-get-shortcode-' + modal_id, {
      ...
      $insert.on('click', function (e) {
        ...
        var shortcode = '';
        var serialize = $modal.find('.ta-forms-field:not(.ta-forms-depend-on)').find(':input:not(.ignore)').serializeObjectTaForms();
    ...
    return this.each(function () {
      ...
      $input.wpColorPicker({
        ...
        change: function (event, ui) {
          ...
          $container.find('.ta-forms--transparent-offset').css('background-color', ui_color_value);
        ...
        create: function () {
          ...
          var a8cIris = $input.data('a8cIris'),
            $transparent_wrap = $('<div class="ta-forms--transparent-wrap">' +
            ...
            $transparent_slider = $transparent_wrap.find('.ta-forms--transparent-slider'),
            $transparent_text = $transparent_wrap.find('.ta-forms--transparent-text'),
            $transparent_offset = $transparent_wrap.find('.ta-forms--transparent-offset'),
            $transparent_button = $transparent_wrap.find('.ta-forms--transparent-button');
          ...
          $transparent_slider.slider({
            ...
            create: function () {
              ...
              $container.on('click', '.wp-picker-default', function () {
                ...
                var default_color = TaForms.funcs.parse_color($input.data('default-color')),
    ...
    return this.each(function () {
      ...
      $siblings.on('click', function () {
        ...
        if (multiple) {
          ...
          $sibling.find('input').prop('checked', true).trigger('change');
          $sibling.addClass('ta-forms--active').siblings().removeClass('ta-forms--active');
  ...
  $.fn.Chat_Help_Pro_help = function() {
    return this.each( function() {
      ...
      $this.on({
        mouseenter: function () {
          $tooltip = $('<div class="ta-forms-tooltip"></div>').html($this.find('.ta-forms-help-text').html()).appendTo('body');

          offset_left = (TaForms.vars.is_rtl) ? $this.offset().left - $tooltip.outerWidth() : ($this.offset().left + 32);
  ...
  $.fn.Chat_Help_Pro_customizer_listen = function (options) {
    ...
    return this.each(function () {
      ...
      var $this = (settings.closest) ? $(this).closest('.ta-forms-customize-complex') : $(this),
        $input = $this.find(':input'),
  ...
  // Window on resize
  //
  TaForms.vars.$window.on('resize ta-forms.resize', TaForms.helper.debounce(function (event) {
  ...
  $(document).ready(function () {
    function updateEmailVars() {
      ...
      if ($messageVars.length && $groupField.length) {
        var fieldInfo = $groupField.find('.ta-forms-cloneable-wrapper').getFieldInfo();
    ...
    $(document).on('keyup input change', '.ta-forms-cloneable-item input[type="text"]', function () {
      updateEmailVars();
    });

    $('.ta-forms-cloneable-wrapper').on('ta-forms.field-info-update', function () {
      updateEmailVars();
    });
  });
  ...
  $(document).on('click', '.wrapper_class_form', function () {
    if ($('.ta-forms--active.wrapper_class_form').length) {
      $('.ta_forms_type_of_whatsapp input[value="number"]').prop('checked', true).trigger('change');
    }
  });
  ...
  $(document).on('click change', '#ta-forms-form .ta-forms-cloneable-helper, #ta-forms-form .ta-forms-cloneable-helper .sp_wgs-icon-clone, #ta-forms-form .ta-forms-cloneable-helper .sp_wgs-icon-drag-and-drop, .ta-forms-cloneable-add', function (e) {
    e.preventDefault();
    var $button = $('#ta-forms-form').find('.ta-forms-save.ta-forms-save-ajax');
    $button.css({ "background-color": "#1a74e4", "pointer-events": "initial" }).val('Save Settings');
  });
  // If no item added Add to assign layout, trigger to add one item.
  setTimeout(() => {
    if ($('#ta-forms-section-assign_layout .ta-forms-cloneable-wrapper .ta-forms-cloneable-item').length == 0 && $('#ta-forms-section-assign_layout .ta-forms-cloneable-add').length > 0) {
      $('#ta-forms-section-assign_layout .ta-forms-cloneable-add').trigger('click');
    }
  }, 500);
